          ...
          bcrypt.hash(newUser.password, salt, (err, hash) => {
            ...
            newUser
              ...
              .then((user) => {
                ...
                host = req.get('host');
                link = process.env.STAFF_BACKEND_URL + "/user/verify?id=" + randKey;
                // console.log(link)
                var mail = {
                  ...
                  html: "<p>You successfully registered an account at " + host + ". Please verify by clicking the link.\n" + link + "</p>",
                };
...
router.post('/forgotPassword', async (req, res) => {
  ...
  let token = SHA256(req.body.email);
  ...
  try {
    if (!data) {
      ...
      link = process.env.STAFF_BACKEND_URL + `/user/reset/?resetPasswordToken=${token}`;
      // console.log("tokenssss", `${token}`, link)
...
router.get('/reset', (req, res) => {
  ...
  User.findOne(condition).then((user) => {
    if (user == null) {
      ...
      res.redirect(process.env.STAFF_FRONTEND_URL + "/resetpass/?resetPasswordToken=" + resetPasswordToken)
    }
  });
});
